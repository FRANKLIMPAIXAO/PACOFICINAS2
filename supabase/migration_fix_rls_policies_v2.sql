-- =====================================================
-- MIGRAÇÃO V2: Refatorar RLS Policies (ORDEM CORRIGIDA)
-- =====================================================
-- Esta migração corrige a vulnerabilidade crítica de segurança
-- NOTA: A função get_user_empresa_id() JÁ EXISTE e usa a tabela usuarios
-- Precisamos apenas recriar as policies (elas já estão corretas)
-- =====================================================
...
-- =====================================================
-- VERIFICAR/RECRIAR FUNÇÃO (já deve existir corretamente)
-- =====================================================

-- A função já existe e está correta, mas vamos garantir
CREATE OR REPLACE FUNCTION get_user_empresa_id()
RETURNS UUID AS $$
BEGIN
    RETURN (SELECT empresa_id FROM usuarios WHERE id = auth.uid() LIMIT 1);
END;
...
SELECT 'RLS Policies recriadas usando a função get_user_empresa_id() que consulta a tabela usuarios' as info;
